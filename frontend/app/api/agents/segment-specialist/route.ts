...
export async function POST() {
  try {
    ...
    const agent = createSegmentSpecialistAgent(supabase)
    ...
    return NextResponse.json({
      ...
      message: result.from_cache
        ? `Knowledge base em cache (segmento: ${result.segmento})`
        : `Segmento detectado: ${result.segmento}`,
      data: result,
    })
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : JSON.stringify(err)
    console.error('[SegmentSpecialistAgent API]', err)
